...
template< typename T>
__global__ void kernel_l0_spar_err( T* dst, const T* d_alphai, const T* d_alphai_neighbor, const T* d_W0_neighbor, const T *d_lambda_l0, uint nd, uint nn)
{
    uint i = blockIdx.x * blockDim.x + threadIdx.x; 
...
__forceinline__ void compute_obj_robust(float& obj, float& l2err, float& l1_spar_err, float& l0_spar_err, float *d_X, uint data_i, float *d_A, float *d_alphai, float *d_alphai_neighbor, 
									   float *d_W0_neighbor, float lambda_l1, float *d_lambda_l0, uint n, uint nn, uint d, cublasHandle_t& handle)
{
	...
	status = cublasSasum(handle, nd, d_alphai, 1, &l1_spar_err);
	if (status != CUBLAS_STATUS_SUCCESS) {
		fprintf (stderr, "!!!! kernel execution error.\n");
		return;
	}
	...
	blocksPerGrid = dim3((nd+threadsPerBlock.x-1)/threadsPerBlock.x,(nn+threadsPerBlock.y-1)/threadsPerBlock.y);
	kernel_l0_spar_err<<<blocksPerGrid, threadsPerBlock>>>(d_l0_spar_err_mat, d_alphai, d_alphai_neighbor, d_W0_neighbor, d_lambda_l0, nd, nn);

	status = cublasSasum(handle, nd*nn, d_l0_spar_err_mat, 1, &l0_spar_err);
	if (status != CUBLAS_STATUS_SUCCESS) {
		fprintf (stderr, "!!!! kernel execution error.\n");
		return;
	}
